...
import { Globe, CheckCircle2, BarChart3, Sparkles, TrendingUp, Search, MapPin, Users, Target, HeartHandshake, ShieldCheck } from "lucide-react";

/* ── Mock Data for Active Initiatives ───────────────── */
const INITIATIVES: (InitiativeCardProps & { region: string; goal: string; audience: string })[] = [
  {
    id: 1,
...
const COMPLETED_INITIATIVES: InitiativeCardProps[] = [
  {
    id: 101,
    entityName: "جمعية نقاء التنموية",
    category: "مياه وصحة",
  ...
  return (
    <DashboardLayout widgets={<CommunityWidgets />}>
      ...
      <div className="px-5 pt-5 pb-4 border-b border-border">
        ...
        <p className="text-[13px] text-slate-500 dark:text-slate-450 mt-0.5 leading-relaxed font-semibold">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
            ...
            <div className="relative flex items-center">
              ...
              <select
                ...
                className="w-full pr-9 pl-4 py-2 rounded-xl border border-slate-200 dark:border-white/10 bg-surface text-[12.5px] text-slate-700 dark:text-slate-350 font-semibold outline-none appearance-none cursor-pointer focus:border-teal-400 dark:focus:border-teal-500"
              >
          ...
          {contributions.map((c, i) => (
            <div key={i} className="bg-surface border border-border rounded-2xl p-5 hover:shadow-md transition-shadow relative">
              ...
              <div className="flex justify-between items-start gap-3 mb-3">
                ...
                <span className="shrink-0 text-[11px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-100 dark:border-emerald-900/40 px-2 py-0.5 rounded-full">
                  مكتمل ومفعّل
                </span>
              </div>
              ...
              <div className="bg-slate-50 dark:bg-slate-900/40 border border-slate-150 dark:border-white/5 rounded-xl p-3.5 text-right space-y-1">
                <span className="text-[10px] font-bold text-slate-450 dark:text-slate-550">الأثر الحقيقي لمساهمتك:</span>
