  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-2 md:mb-4">
        <div className={`w-8 h-8 md:w-10 md:h-10 rounded-xl ${card.iconBg} flex items-center justify-center shrink-0`}>
          <Icon className={`w-4 h-4 md:w-5 md:h-5 ${card.iconColor}`} strokeWidth={2} />
        </div>
        <ArrowUpRight className="w-4 h-4 text-slate-300" />
      </div>
      <p className="text-[11px] font-semibold text-slate-400 mb-0.5">{card.label}</p>
      <p className="text-[18px] md:text-[22px] font-black text-slate-900 leading-tight mb-0.5">{card.value}</p>
      {card.sub && (
...
function ToolCardComponent({ tool, index, onClick }: { tool: ToolCard; index: number; onClick: () => void }) {
  ...
  return (
    <motion.div
      {...fadeUpProps(index + 4)}
      onClick={onClick}
      className="group cursor-pointer bg-surface border border-slate-100 rounded-2xl p-4 md:p-6 hover:shadow-lg hover:-translate-y-1 transition-all duration-200"
    >
      ...
      <h3 className="text-[15px] font-black text-slate-900 mb-1.5 group-hover:text-primary transition-colors">
        {tool.title}
      </h3>
...
function EntityCard({
  ...
  return (
    ...
    >
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-xl shrink-0 ${active ? "bg-primary/10" : "bg-slate-50"}`}>
        {entity.emoji}
      </div>
      <div className="flex-1 min-w-0 text-right">
        <p className={`text-[13px] font-black truncate ${active ? "text-primary" : "text-slate-800"}`}>
          {entity.name}
        </p>
        <p className="text-[11px] text-slate-400 font-semibold">{entity.type}</p>
      </div>
            ...
            {activeView === "hub" ? (
              ...
              >
                ...
                <div>
                  <motion.p
                    {...fadeUpProps(2)}
                    className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-3"
                  >
                    نظرة سريعة
                  </motion.p>
                ...
                <motion.div
                  {...fadeUpProps(11)}
                  className="flex items-center gap-3 p-4 rounded-2xl bg-indigo-50/60 border border-indigo-100"
                >
              ...
              >
                {/* ── Secondary Header ── */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4 lg:pb-3 shrink-0">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        {activeTool && (
                          <div className={`w-8 h-8 rounded-lg ${activeTool.iconBg} flex items-center justify-center shrink-0`}>
                ...
                <div className="flex flex-col lg:flex-row-reverse gap-6 lg:gap-5 lg:flex-1 lg:overflow-hidden mt-6 lg:mt-4">
                          ...
                          className={`flex items-center gap-2.5 transition-all shrink-0 px-4 py-2.5 rounded-full border text-xs md:text-sm font-semibold select-none
                            ...
                            ${
                              isActive
                                ...
                                : "bg-white border-slate-100 hover:bg-slate-50 text-slate-500 hover:text-slate-800 lg:border-transparent lg:bg-transparent"
                            }`}
                        >
                          <Icon className={`w-4 h-4 lg:w-5 lg:h-5 shrink-0 ${isActive ? "text-primary" : "text-slate-400"}`} />
                  ...
                  <div className="w-full lg:w-3/4 lg:h-full lg:flex lg:flex-col lg:overflow-hidden">
                    <div className="bg-white border border-slate-100 rounded-2xl p-4 md:p-6 shadow-sm min-h-[400px] lg:min-h-0 lg:h-full flex flex-col lg:overflow-hidden">
